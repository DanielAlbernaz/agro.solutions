...
class ControllerBannerTerreno extends Controller
{
    function find(Request $request){

        $bannerTerreno = BannerTerreno::find($request->id);
        return view('painel.bannerTerreno.frmAltBannerTerreno', compact('bannerTerreno'));
      }
      ...
      function edit(Request $request){
        try {
            ...
            $objbannerTerreno = BannerTerreno::find($request->id);
            $objbannerTerreno->title = $request->title;
            $objbannerTerreno->imagem =  ($request->image_file ? $request->image_file : $request->imgOld);
            $objbannerTerreno->save();

            /**Log */
            createLog(auth()->user()->id, 'Alterar', 'BannerTerreno',  $objbannerTerreno->id, $_SERVER['REMOTE_ADDR']);

            $retorno = [
                'situacao' => 'success',
                'form' => 'alt',
                'redirect' => 'sistema/editar-bannerTerreno/1',
            ...
            $retorno = [
                'situacao' => 'error',
                'form' => 'alt',
                'redirect' => 'editar-bannerTerreno/1',
