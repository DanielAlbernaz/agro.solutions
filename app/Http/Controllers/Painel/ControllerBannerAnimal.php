...
class ControllerBannerAnimal extends Controller
{

 function find(Request $request){

    $bannerAnimal = BannerAnimal::find($request->id);
    return view('painel.bannerAnimal.frmAltBannerAnimal', compact('bannerAnimal'));
  }
  ...
  function edit(Request $request){
    try {
        ...
        $objbannerAnimal = BannerAnimal::find($request->id);
        $objbannerAnimal->title = $request->title;
        $objbannerAnimal->imagem =  ($request->image_file ? $request->image_file : $request->imgOld);
        $objbannerAnimal->save();

        /**Log */
        createLog(auth()->user()->id, 'Alterar', 'BannerAnimal',  $objbannerAnimal->id, $_SERVER['REMOTE_ADDR']);

        $retorno = [
            'situacao' => 'success',
            'form' => 'alt',
            'redirect' => 'sistema/editar-bannerAnimal/1',
        ...
        $retorno = [
            'situacao' => 'error',
            'form' => 'alt',
            'redirect' => 'editar-bannerAnimal/1',
